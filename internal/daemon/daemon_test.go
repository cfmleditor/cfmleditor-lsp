...
func TestServeMultipleClients(t *testing.T) {
	...
	go func() { _ = Serve(ctx, sock, logger, idx, ct, nil, nil, nil) }()
...
func TestProxyConnectsToExistingDaemon(t *testing.T) {
	...
	go func(){ _ = Serve(ctx, sock, logger, idx, nil, nil, nil, nil) }()
	waitForSocket(t, sock)
...
func TestMultipleConnectionsShareIndex(t *testing.T) {
	...
	go func() { _ = Serve(ctx, sock, zap.NewNop(), idx, ct, nil, nil, nil) }()
	waitForSocket(t, sock)
