...
func TestServeMultipleClients(t *testing.T) {
	...
	go func() { _ = Serve(ctx, sock, logger, idx, ct, nil, nil, nil, nil, nil, nil, server.FormattingConfig{}) }()
...
func TestProxyConnectsToExistingDaemon(t *testing.T) {
	...
	go func(){ _ = Serve(ctx, sock, logger, idx, nil, nil, nil, nil, nil, nil, nil, server.FormattingConfig{}) }()
	waitForSocket(t, sock)
