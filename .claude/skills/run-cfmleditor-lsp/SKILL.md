...
Go 1.26.8 (pinned in `go.mod`) with CGO enabled — the tree-sitter-cfml grammar is a cgo package. No other runtime deps; the binary is self-contained.
...
- **Daemon mode** activates only when a `.cfmleditor.json` config file exists in the current directory or any ancestor directory up to the filesystem root. Without it the server runs standalone (no socket).
